...
def describe_edits(edit_matrix: tuple,
    ...
    if all(char not in list(target_word) for char in list(original_word)):
        ...
        def by_row_description(original_word, target_word, common_chars_row, row_indices):
            steps_1 = []
            if common_chars_row:
                ...
                if remaining_targ and remaining_orig:
                    ...
                    steps_1 += ['remove {}'.format(char) for char in remaining_orig[::-1]]
            return steps_1

        def by_col_description(original_word, target_word, common_chars_col, col_indices):
            steps_2 = []
            if common_chars_col:
                # before common strings
        ...
        common_chars_row, common_chars_col, row_indices, col_indices = find_common_strings(edit_matrix,
                                                                                           original_word,
                                                                                           target_word)
